...
if (process.argv.length !== 3) {
  console.error('USAGE: train-eye-model.js <input_dir>')
  console.error(process.argv)
  process.exit(1)
}

const IMAGE_SIZE = 30 * 30
const LABEL_SIZE = 2

async function run() {
  const input = process.argv[2]
  const labelsAsJson = require(path.join(input, '0000-labels.json'))

  const PREPROCESSED_IMAGES_FILE = path.join(input, '0000-images.uint8')
  const PREPROCESSED_LABELS_FILE = path.join(input, '0000-labels.uint8')

  async function loadImages() {
    const openEyeFiles = new Set(labelsAsJson.filter(item => item.label === 'open').map(item => item.file))
    const notOpenEyeFiles = new Set(labelsAsJson.filter(item => item.label === 'not-open').map(item => item.file))
    console.log(`${openEyeFiles.size} open eyes, ${notOpenEyeFiles.size} not open eyes`)
    ...
    const images = []
    const allFiles = [...openEyeFiles, ...notOpenEyeFiles]
    await Bluebird.map(allFiles, async (file, index) => {
      const filePath = path.join(input, file)
      const rawImage = fs.readFileSync(filePath)
      const imageData = await sharp(rawImage).greyscale().normalize().raw().toBuffer({resolveWithObject: true})

      const labelBytes = [0, 0]
      if (openEyeFiles.has(file)) labelBytes[1] = 1
      else labelBytes[0] = 1
  ...
  const [dataX, dataY] = tf.tidy(() => {
    ...
    return [xs.reshape([n, 30, 30, 1]), labels]
  })
  ...
  await model.save(`file://${path.join(MODEL_DIR, 'eye-model')}`)
}
